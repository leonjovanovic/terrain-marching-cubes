...
public class AddTerrain : MonoBehaviour
{
    ...
    public int terrainBrushSensitivity = 16;
            ...
            for (int j = (int)(Mathf.Max(coord.y - radiusScaled, 0)); j < Mathf.Min(coord.y + radiusScaled + 1, y + 1); j++)//y osa
            {
                ...
                for (int k = (int)(Mathf.Max(coord.z - radiusScaled, 0)); k < Mathf.Min(coord.z + radiusScaled + 1, z + 1); k++)
                {
                    ...
                    if ((valX + valY + valZ) <= r)
                    {
                        if (add && j!= y)
                        {
                            coords_vals[i, j, k] -= Mathf.Sqrt(r - (valX + valY + valZ))/ terrainBrushSensitivity; //0.5f;//isosurface - 0.01f;
                        }
                        if (!add && j!= 0)
                        {
                            coords_vals[i, j, k] += Mathf.Sqrt(r - (valX + valY + valZ)) / terrainBrushSensitivity; //0.5f;// = isosurface + 0.01f;
    ...
    public void OnSliderBrushStrengthValueChanged(float value)
    {
        int val = 16;
        switch (value)
        {
        ...
        terrainBrushSensitivity = val;
        GameObject.Find("BrushSensitivityText").GetComponent<TextMeshProUGUI>().text = terrainBrushSensitivity.ToString();
    }
}
